        ...
        btnInicio.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View view) {
                if (!usuario.getText().toString().isEmpty() && !clave.getText().toString().isEmpty()) {
                    ...
                    Alert.show(getBaseContext(), "error", "Campos vacíos", Toast.LENGTH_SHORT, (ViewGroup) getLayoutInflater().inflate(R.layout.toast, findViewById(R.id.toastCustom)));
                }
            }
        });
    }
            ...
            public void onSuccess(int statusCode, Header[] headers, byte[] responseBody) {
                if (statusCode == 200) {
                    if (DaoUsuario.obtenerList(new String(responseBody)).size() > 0) {
                        ...
                        setPreferencias();
                        Alert.show(getBaseContext(), "exito", "¡Hola! @" + MainActivity.usuario.getUsuario(), Toast.LENGTH_SHORT, (ViewGroup) getLayoutInflater().inflate(R.layout.toast, findViewById(R.id.toastCustom)));
                        Intent intent = new Intent(getBaseContext(), MainActivity.class);
                        intent.addFlags(Intent.FLAG_ACTIVITY_NEW_TASK);
                        getBaseContext().startActivity(intent);
                        finish();
                    } else {
                        Alert.show(getBaseContext(), "error", "Credenciales Incorrectas", Toast.LENGTH_SHORT, (ViewGroup) getLayoutInflater().inflate(R.layout.toast, findViewById(R.id.toastCustom)));
                    }
                }
            }
